...
def player_input(grid, symbol):
    dimensions = len(grid)
    while True:
        user_move = input(f"Player {symbol}, enter your move (e.g., a1): ").lower()
...
def game_play():
    game_type = input("Enter 2 for a two-player game or 4 for a four-player game: ")
    if game_type == '2':
        dimensions = 5
        participants = ['X', 'O']
        victory_condition = 3
    ...
    while True:
        display_grid(grid)
        player_input(grid, participants[player_turn])
        if assess_victory(grid, participants[player_turn], victory_condition):
            display_grid(grid)
            print(f"Player {participants[player_turn]} wins!")
            break
